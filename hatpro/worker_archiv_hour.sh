...
if [ "$#" -ne 2 ]; then
    echo -e "\nNeed a date i.e. 20211213 and hour with padding zero"
    echo -e "bash worker_archiv.sh 20211213 05\n"
    exit 2
fi
...
PATH_TO_RAWDATA_WITH_HOUR=${PATH_TO_RAWDATA}"/H"$2
echo ${PATH_TO_RAWDATA_WITH_HOUR}
${PATH_TO_PYTHON2}/python ${PATH_TO_HATPRO}/masterfile.py ${PATH_TO_RAWDATA} ${PATH_TO_HATPRO} $2 && ${PATH_TO_PYTHON3}/python ${PATH_TO_IMGIDIR}/modules/insertHatpro.py ${PATH_TO_RAWDATA_WITH_HOUR}

#dont need it after isnerting to the DB
if [ -e ${SQLITE_DB_FILE} ]
then
    /bin/rm ${SQLITE_DB_FILE}
fi
