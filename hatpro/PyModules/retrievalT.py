...
# - DATE:        2014-03-06
# -------------------------------------------------------------------
# - DESCRIPTION: retrieval for Temperature 
# -------------------------------------------------------------------
...
class retrievalT( object ):
    ...
    def __init__(self,config,periods,db):
        ...
        #   There are also quadratic terms. x_exp contains the exponent.
        ...
        # - Result to database
        ####self.db.write_RESULT_to_db(RESULT,'RESULT_T')
    ...
    def get_beta_matrix(self):
        ...
        #   For the temperature profile (and we are in the retreival for T)
        #   the retrieval_type has to be 0
        if not int(self.coef.retrieval_type()) == 0:
            utils.exit('Found fancy/wrong retrieval type for temperature profile!')
        ...
        # -----------------------------------------------------------
        # - Decide which retrieval type it is. RT (retrieval type 1) has
        ...
        if not 'OS' in content.keys():
            ...
            beta = content['OS'][0]
            x_info[0].append('offset'); x_info[1].append(-999); x_exp.append(1)
        ...
        if 'SL' in content.keys() and len(content['SL']) > 0:
            ...
            for sensor in ['TS','HS','PS','IS']:
                if int(content[sensor][0]) == 1:
                    x_info[0].append( sensor ); x_info[1].append( -999 ); x_exp.append(1)
        ...
        if 'TL' in content.keys() and len(content['TL']) > 0:
            ...
            for block in range(len(content['TL'])):
                ...
                if content['TL'][0].shape[1] == 1:
                    x_info[0].append( coef_dimension[0] );  x_info[1].append( coef_dimension[block] )
                    x_exp.append(1)
                    ...
                    for elem in coef_dimension:
                        x_info[0].append( elem ); x_info[1].append( coef_dimension[block] )
                        x_exp.append(1)
    ...
    def get_predictor_matrix(self,x_info,x_exp):
        ...
        def get_BLB_values(info,exp):
            ...
            #   time periode (timestamp) and variable.
            print self.BLB[0]
            print self.coef.frequencies()
            print self.coef.angles()
            print self.coef.continuouse.keys()
            sys.exit()
        ...
        for i in range(len(x_info)):
            ...
            if info == 'offset':
                ...
                x_matrix[:,i] = get_BLB_values(info,float(x_exp[i]))
    ...
    def __load_coefficients__(self):
        ...
        print '\n* Reading the coefficients file for the temperature profile'
        print '  File is: %s' % self.config['coef_T']
        self.coef = coefparser({'infile':self.config['coef_T']})
